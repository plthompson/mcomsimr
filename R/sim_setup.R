...
#' @export
#'
env_generate <- function(landscape, env1Scale = 500, timesteps = 1000, plot = TRUE){
  repeat {
    ...
    env.df <- data.frame(env1 = vegan::decostand(RF,method = "range"), x= landscape$x, y = landscape$y, time = rep(1:timesteps, each = nrow(landscape)))
    env.initial <- env.df[env.df$time == 1,]
    if((max(env.initial$env1)-min(env.initial$env1)) > 0.6) {break}
  }

  if(plot == TRUE){
    g<-ggplot2::ggplot(env.df, aes(x = time, y = env1, group = interaction(x,y), color=interaction(x,y)))+
      ggplot2::geom_line()+
